...
if [[ "${ENVIRONMENT}" == "production" ]]; then
  ...
  # CRITICAL FIX: Prevent sqlite3 VACUUM from filling disk on restart loops
  # The start script runs: echo "VACUUM INTO ..." | sqlite3 "$DATABASE_FILE"
  # Each VACUUM creates a full copy of the DB. Crash loops fill the disk.
  # Replace sqlite3 with a no-op so the VACUUM is silently skipped.
  mkdir -p /tmp/bin
  printf '#!/bin/sh\nexit 0\n' > /tmp/bin/sqlite3
